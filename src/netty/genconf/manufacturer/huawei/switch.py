...
from netty.genconf.factory.switch_factory import Switch, SwitchFactory, Stp, VlanIf
...
class HuaweiSwitch(SwitchFactory):
    default_jinja_template_path: ClassVar[Path] = Path(
        f"{PROJECT_DIR}/src/netty/genconf/manufacturer/huawei/"
    )
    ...
    def __management_ip(self, vlan_ifs: list[VlanIf]) -> IPv4Address | None:
        for vif in vlan_ifs:
