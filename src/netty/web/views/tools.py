...
async def address_group_page(request: Request, type: str = "fqdn"):
    ...
    return templates.TemplateResponse(
        "address_group.html", {"request": request, "type": type}
    )
...
async def address_group_generate(
    ...
    if type == "fqdn":
        ...
        result = {"fqdns": items, "group_name": group_name, "cache_ttl": cache_ttl}
    else:
        items = [s.strip() for s in (subnets or "").split("\n") if s.strip()]
        result = {"subnets": items, "group_name": group_name}

    return templates.TemplateResponse(
        "address_group.html", {"request": request, "type": type, "result": result}
    )
