...
def gen_template_xlsx(region: str, path: Path) -> None:
    ...
    for col_index, header in enumerate(headers, start=1):
        ...
        hardware_sheet.column_dimensions[get_column_letter(col_index)].width = (
            max_length + 15
        )
    ...
    interconnections_fill = PatternFill(
        start_color="FFFF00", end_color="ADD8E6", fill_type="solid"
    )
    for col_index, header in enumerate(conn_headers, start=1):
        ...
        max_length = len(header)
        interconnections_sheet.column_dimensions[get_column_letter(col_index)].width = (
            max_length + 15
        )

    subnets_sheet = workbook.create_sheet(TemplateName.subnet_sheet)
    subnets_fill = PatternFill(
        start_color="00FF00", end_color="ADD8E6", fill_type="solid"
    )
    for col_index, header in enumerate(generate_default_subnet_headers(), start=1):
        ...
        max_length = len(header)
        subnets_sheet.column_dimensions[get_column_letter(col_index)].width = (
            max_length + 15
        )
    ...
    for col_index, header in enumerate(generate_default_fix_ip_headers(), start=1):
        ...
        cell.fill = subnets_fill
        max_length = len(header)
        fix_ips_sheet.column_dimensions[get_column_letter(col_index)].width = (
            max_length + 15
        )
