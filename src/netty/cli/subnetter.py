...
class Subnetter:
    ...
    def __init__(self, subnet: str, site_code: str, num_wired_endpoints: int, num_wireless_aps: int) -> None:
    ...
    def allocate(self)->list[Subnet]:
        ...
        for vlan in self.template():
            subnet_size = IPv4Network(f'0.0.0.0/{vlan.subnet_size}', strict=False).prefixlen
            subnet: IPv4Network
            while True:
                subnet = IPv4Network(f'{str(current_network)}/{subnet_size}', strict=False)
                overlap = False
            ...
            results.append(
                Subnet(
                    ...
                    DHCPRangeStart=subnet.network_address + vlan.reserved_pool_addrs+1 if vlan.dhcp_pool else None,
                    DHCPRangeEnd=subnet.broadcast_address-1 if vlan.dhcp_pool else None,
                    IfAddress=IPv4Interface(f"{subnet.network_address+1}/{subnet.prefixlen}"),
                )
            )
        return results
